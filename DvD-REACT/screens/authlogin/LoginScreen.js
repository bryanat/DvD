...
export default function LoginScreen({navigation}) {
    ...
    // method 1 (of two methods) to scale image is being used below method 2 is
    // other method is style={{resizeMode: 'contain', flex:1}} then wrap that in a
    // view with style={{height: screenHeight*0.35}}) 0.35 is scale
    ...
    function pressLoginButton() {
        ...
        if (emailState.match(/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i) != null) {
            ...
            // if password has at least 8 characters, one letter, one number (checked via
            // regex match)
            if (passwordState.match(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/) != null) {
                ...
                setEmailValidityState('')
                axios
                    .put('http://192.168.1.147:8088/logins/login', {
                        email: emailState,
                        password: passwordState
                    })
                    // if response from server is true then set react auth Context to loggedIn true
                    .then(function (response) {
                        // could be a ternary operator instead... response.loggedIn == true ?
                        // AuthContext.loggedIn = true : AuthContext.loggedIn = false
                        if (response.data.authenticated == true) {
                            // set auth context to loggedIn = true causes navigation to switch to
                            // RootNavigator and move to home screen
                ...
                setPasswordValidityState(
                    "Enter a valid password containing at least one letter and one number"
                )
                console.log("enter a valid password")
            }
        } else {
            setEmailValidityState(
                "Enter a valid email address in the format: blahblahblah@mail.com"
            )
